import { addDays, addMonths, getDaysInMonth, isSameDay, isBefore } from 'date-fns'
import { daysOfWeek, toRange, dayOffset, getWeek, getMonth, getCalendar } from '../lib/utils'
...
describe('toRange', () => {
  ...
  it('returns a date range object, ignoring time information', () => {
    ...
    expect(isSameDay(range.startDate, startDate) && isSameDay(range.endDate, endDate)).toBe(true)
  })
...
describe('getMonth', () => {
  ...
  it('skips x days from the start and limits to a set maximum number of days', () => {
    for (let i = 0; i < nOfMonths; i++) {
      ...
      expect(getMonth(date, skip, limit)).toHaveLength(Math.min(limit, daysInMonth - skip))
    }
  })
})
